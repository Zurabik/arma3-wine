Сервер АРМА3 для linux отличается от сервера для windows, многие (но не все) моды используют разный регистр в путях и не работают в linux, но при этом используя wine возможно запускать их в системах linux. 
